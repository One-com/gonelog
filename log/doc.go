...
The default Logger supports all this as well, using log level constants source code compatible with
...
Logging with key/value data is (in its most simple form) done by calling level specific functions. First argument is the message, subsequence arguements key/value data:
...
The other is to pick an comma-ok style log function:
    	...
    	if f,ok := l.DEBUGok(); ok  { f("haevy", "fib123", fib(123)) }

Loggers can have names, placing them in a global "/" separated hierachy. It's recommended to create a Logger by mentioning it by name using GetLogger("logger/name") - instead of creating unnamed Loggers with NewLogger().
...
When Logging an event at a Logger the tree of Loggers by name are only traversed to find the first Logger having a Handler attached. The log-event is then sent to that handler. The Python behaviour is to send the event to all Handlers found in the Logger tree. This is not the way it's done here. Only one Handler will be given the event to log. If you wan't more Handlers getting the event, use a MultiHandler.
